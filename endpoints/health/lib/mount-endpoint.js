...
function toDescriptive(specs) {
    ...
    Object.keys(queries).forEach(function (queryKey) {
        var url = '/' + queryKey;
...
function toDescriptiveMethod(schemas) {
    ...
    Object.keys(schemas).forEach(function addSchema(version) {
        ...
        console.log('methodSchema', methodSchema);
...
function toDescriptiveHandler(handler) {
    return function (typedRequest, opts, cb) {
        handler(typedRequest, opts, function (err, value) {
            if (err) {
                return cb(err);
            }
